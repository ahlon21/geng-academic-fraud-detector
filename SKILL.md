...
| 检测项 | LLM 结论 | 工具验证 | 一致？ |
|-------|---------|---------|-------|
| Fig X/Y 数据复用 | 怀疑 | Benford: 偏离, SSIM: 0.97 | ✅ |
| Table N SD 一致性 | 未发现 | Levene p=0.92, CV=0.08 | ✅ |
| p 值分布 | 正常 | p-hacking zone: 12% | ✅ |

## 置信度评估

- **Benford 检测**：MAD=X（判定），末位偏斜率=X（判定）
- **图片 SSIM**：最高 X，判定
- **GRIM 检验**：通过/未通过
- **p 值分布**：p-hacking 区间占比 X%
...
1. **图像分析增强**：本 skill 现配备 `scripts/fraud_tools.py`，支持 SSIM 像素级比对、ELA 误差分析、Canny 边缘泳道拼接检测。对于已提取的图片文件，工具可提供像素级证据。PDF 内嵌图片需先导出为 PNG/TIFF。
