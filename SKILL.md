...
遇到 PDF 图片模糊、扫描缺页、文字识别不全等情况时，显式标记信息缺口：
- 标记 `[MATERIAL GAP]` + 缺口说明（如"图 3A 分辨率过低，无法判断泳道拼接 [MATERIAL GAP]"）
- 不在缺口处用"可能""似乎"模糊带过，标记后如实继续
- 缺口罩盖范围：图片质量、截断的表格、未提供的 supplementary 材料
...
生成报告后、交付给用户前，按以下流程完成交付前检查：

**退出协议：** 根据本轮分析的结果确定状态码，在报告末尾注明：
- `DONE` — 分析完整完成，所有可检测维度均已覆盖
- `DONE_WITH_CONCERNS` — 已完成，但有 {N} 处信息缺口未填补
- `BLOCKED` — 无法继续（PDF 无法读取、格式不支持等），说明阻塞原因

**完整性自检：** 交付前快速过一遍：
- [ ] M1 参数填充：报告中有没有论文里不存在的内容？
- [ ] M2 未核实引用：声明的结论是否有论文内的证据支撑？
- [ ] M3 结论跳跃：是否从局部异常跳到了"系统性造假"？
- [ ] M4 捷径依赖：有没有更严谨的路径没走？
- [ ] M5 错误重构：有没有把可能的误报表达为确凿结论？
- [ ] M6 方法捏造：是否声称使用了实际未执行的检测方法？
- [ ] M7 框架锁定：是否只在六式框架内思考，忽略了其他可能？

超过 3 项未勾选时，退回 Step 4 重新评估。

**Material Passport：** 在报告开头嵌入溯源元数据：

```yaml
---
produced_by: hanako / geng-academic-fraud-detector
timestamp: {分析时间}
source: {论文标题 / DOI / 文件路径}
status: fully_analyzed / partial / blocked
gap_count: {缺口数量}
---
```
